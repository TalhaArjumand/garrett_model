...
@dataclass(frozen=True)
class InternalToExternalTypeDCandidate:
    symbol: str
    timeframe: str
    direction: Direction
    irl_lower_bound: float
...
@dataclass(frozen=True)
class ExternalToInternalTypeDCandidate:
    symbol: str
    timeframe: str
    direction: Direction
    erl_kind: str
...
def _build_type_d_candidate(
    *,
    ...
    return InternalToExternalTypeDCandidate(
        symbol=c1.symbol,
        timeframe=c1.timeframe,
        direction=direction,
        irl_lower_bound=irl.lower_bound,
        ...
        decision_time_safe=True,
        reason=(
            f"Valid {direction} Type D inside-candle sequence formed after a "
            f"{direction} IRL/FVG {timing_phrase}. The active IRL was touched "
            "through C1 or C2, remained eligible through C3 close, and the "
            "delayed inside-candle Type D structure held."
        ),
    )
...
def detect_internal_to_external_type_d_candidates(
    candles: Iterable[Any],
    *,
    max_wick_fraction: float = 0.25,
) -> list[InternalToExternalTypeDCandidate]:
    validated = _validate_series(candles)
    if len(validated) < 3:
        return []

    candidates: list[InternalToExternalTypeDCandidate] = []
    for idx in range(len(validated) - 2):
        c1, c2, c3 = validated[idx], validated[idx + 1], validated[idx + 2]

        try:
            bullish_sequence = is_bullish_type_d(
                c1,
                c2,
                c3,
                max_lower_wick_fraction=max_wick_fraction,
        ...
        try:
            bearish_sequence = is_bearish_type_d(
                c1,
                c2,
                c3,
                max_upper_wick_fraction=max_wick_fraction,
...
def count_internal_to_external_type_d_sequences(
    candles: Iterable[Any],
    *,
    max_wick_fraction: float = 0.25,
) -> tuple[int, int]:
    candidates = detect_internal_to_external_type_d_candidates(
        candles,
        max_wick_fraction=max_wick_fraction,
    )
    bullish = {
...
def _build_external_type_c_candidate(
    *,
    ...
    return ExternalToInternalTypeCCandidate(
        symbol=c1.symbol,
        timeframe=c1.timeframe,
        direction=direction,
        erl_kind=erl.kind,
        ...
        decision_time_safe=True,
        reason=(
            f"Valid {direction} external-to-internal Type C c3-closure sequence "
            f"formed after a same-bias ERL that {timing_phrase}. The live ERL was "
            "touched through C1 or C2, strict Type A and Type B branches were "
            "excluded, and the first strong confirming closure arrived on C3 while "
            "target-side logic remains intentionally excluded from this bridge."
        ),
    )


def _build_external_type_d_candidate(
    *,
    erl: ERLCandidate,
    c1: Candle,
    c2: Candle,
    ...
    return ExternalToInternalTypeDCandidate(
        symbol=c1.symbol,
        timeframe=c1.timeframe,
        direction=direction,
        erl_kind=erl.kind,
        ...
        decision_time_safe=True,
        reason=(
            f"Valid {direction} external-to-internal Type D inside-candle sequence "
            f"formed after a same-bias ERL that {timing_phrase}. The live ERL was "
            "touched through C1 or C2, strict Type A / Type B / Type B additive "
            "extension / Type C branches were excluded by the family primitive, "
            "and target-side logic remains intentionally excluded from this bridge."
        ),
    )
...
def detect_external_to_internal_type_c_candidates(
    candles: Iterable[Any],
    *,
    max_wick_fraction: float = 0.25,
) -> list[ExternalToInternalTypeCCandidate]:
    validated = _validate_series(candles)
    if len(validated) < 3:
        return []

    candidates: list[ExternalToInternalTypeCCandidate] = []
    for idx in range(len(validated) - 2):
        c1, c2, c3 = validated[idx], validated[idx + 1], validated[idx + 2]
        ...
        if (
            bullish_sequence
            and not is_bullish_c2_reversal_to_expansion(
                c1,
                c2,
                max_lower_wick_fraction=max_wick_fraction,
            )
            and not is_bullish_type_b_additive_extension(c1, c2, c3)
        ):
            for erl, touch in _collect_touching_active_directional_erls(
                validated,
                before_index=idx,
                ...
                candidates.append(
                    _build_external_type_c_candidate(
                        erl=erl,
                        c1=c1,
                        c2=c2,
                        c3=c3,
        ...
        if (
            bearish_sequence
            and not is_bearish_c2_reversal_to_expansion(
                c1,
                c2,
                max_upper_wick_fraction=max_wick_fraction,
            )
            and not is_bearish_type_b_additive_extension(c1, c2, c3)
        ):
            for erl, touch in _collect_touching_active_directional_erls(
                validated,
                before_index=idx,
...
def count_external_to_internal_type_c_sequences(
    candles: Iterable[Any],
    *,
    max_wick_fraction: float = 0.25,
) -> tuple[int, int]:
    candidates = detect_external_to_internal_type_c_candidates(
        candles,
        max_wick_fraction=max_wick_fraction,
    )
    bullish = {
...
def detect_external_to_internal_type_d_candidates(
    candles: Iterable[Any],
    *,
    max_wick_fraction: float = 0.25,
) -> list[ExternalToInternalTypeDCandidate]:
    validated = _validate_series(candles)
    if len(validated) < 3:
        return []

    candidates: list[ExternalToInternalTypeDCandidate] = []
    for idx in range(len(validated) - 2):
        c1, c2, c3 = validated[idx], validated[idx + 1], validated[idx + 2]
        ...
        if bullish_sequence:
            for erl, touch in _collect_touching_active_directional_erls(
                ...
                candidates.append(
                    _build_external_type_d_candidate(
                        erl=erl,
                        c1=c1,
                        c2=c2,
                        c3=c3,
...
def count_external_to_internal_type_d_sequences(
    candles: Iterable[Any],
    *,
    max_wick_fraction: float = 0.25,
) -> tuple[int, int]:
    candidates = detect_external_to_internal_type_d_candidates(
        candles,
        max_wick_fraction=max_wick_fraction,
    )
    bullish = {
